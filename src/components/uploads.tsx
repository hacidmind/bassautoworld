...
import { imageFileAccept, imageFormatLabel } from "@/lib/image-formats";
import {
  parseImageUrls,
  runUploadBatch,
  uploadPhoto,
...
export function Uploads({
  ...
  onPendingUrlsChange?: (pending: boolean) => void;
}) {
  ...
  async function upload(sources: (File | string)[], urls = false) {
    ...
    if (sources.length + value.length > (review ? 1 : 30)) {
      setError(
        review
          ...
          : "You can add " +
              (30 - value.length) +
              " more photos. Remove some URLs or photos and try again.",
      );
      return;
    }
    ...
    try {
      await runUploadBatch(
        sources,
        (source) => uploadPhoto(source, review),
        (index, result) => {
          completed++;
          ...
          onChange(
            [...value, ...results.filter((v): v is UploadImage => !!v)].map(
              (v, i) => ({ ...v, sortOrder: i }),
            ),
          );
          setProgress(
            "Processed " + completed + " of " + sources.length + " photos...",
          );
        },
      );
      setFailed(failures);
      if (urls) {
        setImageUrls(failures.map((f) => String(f.source)).join("\n"));
        onPendingUrlsChange?.(failures.length > 0);
      }
      setProgress(
        sources.length -
          failures.length +
          " photo(s) added" +
          (failures.length
            ? "; " + failures.length + " failed. Successful photos are kept."
            : ".") +
          (review ? "" : " Click Save vehicle to update the website gallery."),
      );
    } finally {
  ...
  return (
    <div className="wide photo-upload" aria-busy={busy}>
      ...
      <p className="muted">
        {review ? "One photo" : "Up to 30 photos"}, 15 MB each.{" "}
        {imageFormatLabel}.
      </p>
      {!review && (
        <div className="url-upload">
          <label className="field">
            ...
            <textarea
              ...
              rows={5}
              placeholder={
                "https://example.com/front.jpg\nhttps://example.com/interior.avif"
              }
              onChange={(e) => {
                setImageUrls(e.target.value);
                onPendingUrlsChange?.(!!e.target.value.trim());
              }}
            />
          </label>
          <p className="muted">
            Paste one direct image link per line. Up to three photos upload at
            once. Duplicate links in this batch are ignored.
          </p>
          <button
            type="button"
            disabled={busy || !imageUrls.trim()}
            onClick={() => upload(parseImageUrls(imageUrls), true)}
          >
            Add photos from URLs
          </button>
        </div>
      ...
      {!!failed.length && (
        <div className="error" role="alert">
          ...
          <button
            ...
            onClick={() =>
              upload(
                failed.map((f) => f.source),
                failed.every((f) => typeof f.source === "string"),
              )
            }
          >
            Retry failed photos
